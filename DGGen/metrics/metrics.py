...
from metrics.tgg_utils import Edge
...
def compute_graph_statistics(A_in, Z_obs=None):
    ...
    if claw_count != 0:
        statistics["clustering_coefficient"] = (
            3 * statistics["triangle_count"] / claw_count
        )
    else:
...
def create_timestamp_edges(
    graphs, timestamps
):  # Club the graph snapshot with its timestamp
    edge_lists = []
    node_set = set()
...
def calculate_temporal_katz_index(
    graphs, node_set, decay_f, No, Beta
):  # edges is a list of timestamped interactions ### Assume the edges are directional in nature
    graphs = graphs.copy()
    node_incoming_edges_dict = (
        {}
    )  # it will keep only which have occured before time t (strictly less)
    katz_index = {}
    ...
    for index, gt in enumerate(graphs):
        ...
        for edge in gt:
            ...
            edge.w = rv + 1
            current_node_incoming_edges_dict = update_dict(
                current_node_incoming_edges_dict, u, edge
            )
