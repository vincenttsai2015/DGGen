...
def convert_graph_from_defauldict_to_dict(
    graph,
):  # Assuming each key contains a defaultdict graph too
    for key, _ in graph.items():
        graph[key] = dict(graph[key])
    return dict(graph)


def get_adj_graph_from_random_walks(
    random_walks, start_time, end_time, undirected=True
):
...
def get_binary_adj_graph_from_sampled_temporal_adj_matrix(
    graph, start, end, edge_count_needed, undirected=True
):
    ...
    for time in range(start, end + 1):
        for start_node, adj_list in graph[time].items():
            for end_node, count in adj_list.items():
                if start_node != end_node:
                    if undirected:
                        key = get_unique_string_from_edge_tuple(
    ...
    print(get_total_edges(graph) * 0.5)
    return graph


def get_adj_origina_graph_from_original_temporal_graph(
    temporal_original_graph, start_time, end_time
):
    graph = defaultdict(lambda: defaultdict(lambda: 0))
...
def get_total_edges_from_temporal_adj_list_in_time_range(
    temporal_adj_matrix, start_time, end_time
):
    ct = 0
    graph = defaultdict(lambda: defaultdict(lambda: 0))
...
def get_total_nodes_and_edges_from_temporal_adj_list_in_time_range(
    temporal_adj_matrix, start_time, end_time
):
    ct = 0
    graph = defaultdict(lambda: defaultdict(lambda: 0))
...
def get_binary_adj_graph_from_sampled_temporal_adj_matrix_using_node_wise_counts(
    graph, start, end, edge_count_needed, node_count_needed, undirected=True
):
    required_edge_counts = edge_count_needed
    print(
        "Required edges count and node count , ",
        required_edge_counts,
        node_count_needed,
    )
    ...
    for time in range(start, end + 1):
        for start_node, adj_list in graph[time].items():
            for end_node, count in adj_list.items():
                if (
                    start_node != end_node
                ):  # and start_node in required_nodes and end_node in required_nodes :
                    if undirected:
                        key = get_unique_string_from_edge_tuple(
                            start_node, end_node, time
                        )
    ...
    print(
        "Acquired edges and nodes till first stage",
        get_total_edges(graph) * 0.5,
        len(final_nodes),
    )
...
def sample_adj_graph_havel_hakimmi(
    graph,
    start,
    end,
    edge_count_needed,
    ...
    print(
        [item[1] for item in current_deg_seq],
        np.sum([item[1] for item in current_deg_seq]) / 2,
    )
    ...
    print(
        [item[1] for item in updated_deg_seq],
        np.sum([item[1] for item in updated_deg_seq]) / 2,
    )
    print("nodes and edges", get_edges_nodes_graph(graph))
...
def sample_adj_graph_multinomial_k_edges(
    graph,
    start,
    end,
    edge_count_needed,
...
def sample_adj_graph_multinomial_k_inductive(
    graph,
    start,
    end,
    edge_count_needed,
